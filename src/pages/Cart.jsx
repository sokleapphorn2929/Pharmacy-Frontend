...
export default function Cart() {
  ...
  const handleCheckout = async () => {
    ...
    try {
      showStatus(
        "Processing",
        "Placing your order and initializing payment...",
        "info",
      );
      ...
      await Promise.all(
        cartItems.map((item) => API.delete(`/cards/${item.id}`)),
      );
      ...
      showStatus(
        "Order Placed",
        "Your order has been placed. Please wait for admin verification of your payment.",
        "success",
      );
    } catch (error) {
  ...
  const handleUpdateQuantity = async (
    ...
    try {
      ...
      showStatus(
        "Update Failed",
        "Could not change item quantity. Please try again.",
        "error",
      );
    } finally {
      setUpdatingId(null);
  ...
  const executeRemoveItem = async (cartId) => {
    ...
    try {
      ...
      showStatus(
        "Item Removed",
        "The product has been successfully removed from your bag.",
        "success",
      );
    } catch (error) {
      ...
      showStatus(
        "Deletion Failed",
        "Failed to delete item from your cart setup.",
        "error",
      );
    }
  };
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8 border-b border-slate-200 dark:border-slate-800/80 pb-5">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Review your selected healthcare formulas and pharmacy items before
            checking out.
          </p>
        </div>
        ...
        {cartItems.length === 0 ? (
          <div className="text-center py-16 bg-white dark:bg-slate-800 border border-dashed border-slate-200 dark:border-slate-700/60 rounded-3xl p-8 max-w-lg mx-auto shadow-sm">
            ...
            <p className="text-xs text-slate-400 dark:text-slate-500 mt-1 mb-6 px-4">
              It looks like you haven't added any medicinal compounds or
              products to your bag yet.
            </p>
            <Link
              to="/kh/home"
