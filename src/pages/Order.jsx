...
export default function Order() {
  ...
  const handleDeleteOrder = async () => {
    ...
    try {
      ...
      setOrders(orders.filter((order) => (order._id || order.id) !== orderToDelete));
      setShowModal(false);
        ...
        {showModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-4">
            <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl max-w-sm w-full shadow-xl">
              <h3 className="text-lg font-bold text-slate-900 dark:text-white">Delete Order?</h3>
              <p className="text-slate-500 dark:text-slate-400 mt-2 mb-6">Are you sure? This action cannot be undone.</p>
              ...
              return (
                ...
                >
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <p className="text-xs font-bold text-slate-400 uppercase">Order ID: #{orderId}</p>
                  ...
                  <div className="mt-4 pt-4 border-t border-slate-200 dark:border-slate-700">
                    <h4 className="text-[10px] font-bold text-slate-400 uppercase mb-3">Ordered Items</h4>
                    <div className="space-y-2">
                      {order.order_items && order.order_items.map((item) => (
                        <div key={item._id || item.id} className="flex justify-between text-sm">
                          <span className="text-slate-700 dark:text-slate-300">
                            {item.products?.product_name || "Product"} <span className="text-slate-400">x{item.qty}</span>
                          </span>
                    ...
                    <div className="mt-4 text-right pt-3 border-t border-dashed border-slate-300 dark:border-slate-700">
                      <span className="font-black text-slate-900 dark:text-white text-lg">
                        Total: ${order.order_items?.reduce((sum, item) => sum + (parseFloat(item.price) * parseInt(item.qty)), 0).toFixed(2)}
                      </span>
                    </div>
                  </div>
                </div>
